...
static void set_thruster_led(pin_t led_pin, uint16_t thruster_value) {
  pin_write(led_pin, thruster_value ? HIGH : LOW);
}

static void update_thruster_leds(chip_state_t *chip) {
  if (chip->control_len < 16) {
    set_thruster_led(chip->thr1_led, 0);
    set_thruster_led(chip->thr2_led, 0);
    set_thruster_led(chip->thr3_led, 0);
    set_thruster_led(chip->thr4_led, 0);
    return;
  }

  set_thruster_led(chip->thr1_led, read_u16_le(chip->control_payload, CTRL_M1_OFFSET));
  set_thruster_led(chip->thr2_led, read_u16_le(chip->control_payload, CTRL_M2_OFFSET));
  set_thruster_led(chip->thr3_led, read_u16_le(chip->control_payload, CTRL_M3_OFFSET));
  set_thruster_led(chip->thr4_led, read_u16_le(chip->control_payload, CTRL_M4_OFFSET));
}
...
void chip_init() {
  ...
  chip->thr1_led = pin_init("THR1_LED", OUTPUT);
  chip->thr2_led = pin_init("THR2_LED", OUTPUT);
  chip->thr3_led = pin_init("THR3_LED", OUTPUT);
  chip->thr4_led = pin_init("THR4_LED", OUTPUT);
...
#if PAYLOAD_VERBOSE
  printf("[payload] chip initialized with SPI API\n");
#endif
}
