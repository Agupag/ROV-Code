...
// Alternate encoding test variant:
// OFF=1, FORWARD=0, REVERSE=2
#define THRUSTER_CMD_OFF 1
#define THRUSTER_CMD_FWD 0
#define THRUSTER_CMD_REV 2
...
void readControlSettings()
{
  ...
  c_data.m1 = THRUSTER_CMD_OFF;
  c_data.m2 = THRUSTER_CMD_OFF;
  c_data.m3 = THRUSTER_CMD_OFF;
  c_data.m4 = THRUSTER_CMD_OFF;
  ...
  if (btnUp && !btnDown)
  {
    c_data.T_VERT = THRUSTER_CMD_FWD;
  }
  else if (btnDown && !btnUp)
  {
    c_data.T_VERT = THRUSTER_CMD_REV;
  }
  ...
  if (y1 < -20)
  {
    if (x1 > 20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_FWD;
      c_data.T_REAR_R = THRUSTER_CMD_OFF;
      c_data.T_FRONT = THRUSTER_CMD_FWD;
    }
    else if (x1 < -20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_OFF;
      c_data.T_REAR_R = THRUSTER_CMD_FWD;
      c_data.T_FRONT = THRUSTER_CMD_FWD;
    }
    else
    {
  ...
  else if (y1 > 20)
  {
    if (x1 > 20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_REV;
      c_data.T_REAR_R = THRUSTER_CMD_OFF;
      c_data.T_FRONT = THRUSTER_CMD_REV;
    }
    else if (x1 < -20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_OFF;
      c_data.T_REAR_R = THRUSTER_CMD_REV;
      c_data.T_FRONT = THRUSTER_CMD_REV;
    }
    else
    {
  ...
  else
  {
    if (x1 > 20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_FWD;
      c_data.T_REAR_R = THRUSTER_CMD_REV;
      c_data.T_FRONT = THRUSTER_CMD_OFF;
    }
    else if (x1 < -20)
    {
      c_data.T_REAR_L = THRUSTER_CMD_REV;
