...
const bool unoLedTestMode = true;
const bool servo2LedActiveLow = true;
...
/***********Pin Assignments********************/
#define CE_PIN 8                    // Uno test mapping
#define CSN_PIN 9                   // Uno test mapping
#define LCD_ADDR 0x27
#define AFOR 4                      // LED outputs for Uno testing
#define BFOR 5
#define CFOR 6
#define DFOR 7
#define AREV 10
...
#define SV1READ A4
#define SV2READ A5
const int chipSelect = 3;                           // Uno test mapping
...
void setup()
{
  ...
  SetAllOriginalStatesToReleased();
  applyControlStatesToLedPins();
  updateServo2LedIndicator(c_data.s2angle);
 ...
 */
{
  if( !unoLedTestMode ) radioCheckAndReply();
  refreshControlData();
  processPayloadData();
}
...
{
  readControlSettings();
  if( !unoLedTestMode ) radio.writeAckPayload(1, &c_data, sizeof_c_data );
}
...
void SetAllOriginalStatesToReleased()
{
  AFOR_state = HIGH;
  AREV_state = HIGH;
  BFOR_state = HIGH;
...
void applyControlStatesToLedPins()
{
  // Virtual state LOW means active control. LEDs are active HIGH.
  digitalWrite(AFOR, AFOR_state == LOW ? HIGH : LOW);
  digitalWrite(AREV, AREV_state == LOW ? HIGH : LOW);
  digitalWrite(BFOR, BFOR_state == LOW ? HIGH : LOW);
  digitalWrite(CFOR, CFOR_state == LOW ? HIGH : LOW);
  digitalWrite(CREV, CREV_state == LOW ? HIGH : LOW);
  digitalWrite(DFOR, DFOR_state == LOW ? HIGH : LOW);
  digitalWrite(DREV, DREV_state == LOW ? HIGH : LOW);
}

void updateServo2LedIndicator(uint16_t rawServo2Angle)
{
  if (usingServo2)
  {
    uint8_t ledValue = constrain(map(rawServo2Angle, 0, 180, 0, 255), 0, 255);
    if (servo2LedActiveLow)
    {
      ledValue = 255 - ledValue;
    }
    analogWrite(BREV, ledValue);
  }
  else
  {
    analogWrite(BREV, 0);
...
*/ 
{
  ...
  uint16_t servo2RawAngle = 90;
  if( usingServo2 ) servo2RawAngle = constrain(map(analog2.y, -128, 127, 0, 180), 0, 180);
  if( usingServo2 ) c_data.s2angle = constrain(servo2RawAngle, 45, 120);
  else c_data.s2angle = 90;
  updateServo2LedIndicator(servo2RawAngle);
