 ...
 * Last-resort production sketch based on FINALCODE with combined x-axis turn input.
 */
...
/*************************************************************/
int combineTurnAxes(short x1, short x2)
{
  return constrain((int)x1 + (int)x2, -255, 255);
}
...
void readControlSettings()
{
  ...
  int turnX = combineTurnAxes(x1, x2);
  ...
  if (y1 < -TURN_DEADBAND)
  {
    if (turnX > TURN_DEADBAND)
    {
      c_data.T_REAR_L = 1;
      c_data.T_REAR_R = 0;
      c_data.T_FRONT = 1;
    }
    else if (turnX < -TURN_DEADBAND)
    {
      c_data.T_REAR_L = 0;
      c_data.T_REAR_R = 1;
      c_data.T_FRONT = 1;
